...
DynamicJsonDocument ApiEncoder::encodeClient(const ClientMeta &client_data, uint16_t runtime_id) {
  ...
  api_version_strm << static_cast<int>(client_data.api_version_major) << "." << static_cast<int>(client_data.api_version_minor) << "." << static_cast<int>(client_data.api_version_bugfix);
...
DynamicJsonDocument ApiEncoder::encodeGadgetUpdate(const GadgetUpdateMeta &data) {
  ...
  //   c_data["type"] = static_cast<int>(c.type);
