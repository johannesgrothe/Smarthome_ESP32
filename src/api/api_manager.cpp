...
void ApiManager::handleEcho(const std::shared_ptr<Request> &req) {
    const DynamicJsonDocument request = req->getPayload();
    req->respond(req->getPath(), request);
};

void ApiManager::handleGadgetUpdate(const std::shared_ptr<Request> &req) const {
    const auto gadget_update_meta = ApiDecoder::decodeGadgetUpdateInfo(req->getPayload());
...
void ApiManager::handleSystemConfigWrite(const std::shared_ptr<Request> &req) const {
    const auto cfg = ApiDecoder::decodeSystemConfig(req->getPayload());
    if (cfg == nullptr) {
        req->respond(false, "Could not decode config properly");
    } else {
        const bool status = delegate_->handleSystemConfigWrite(*cfg);
        if (status) {
...
void ApiManager::publishEvent(const std::shared_ptr<Event> &event) const {
    // TODO: implement event publishing
}

void ApiManager::publishHeartbeat() const {
    auto payload = ApiEncoder::encodeHeartbeat(runtime_id_);

    const auto heartbeat_request = std::make_shared<Request>(api_definitions::uris::heartbeat,
                                                             genRequestID(),
                                                             client_id_,
                                                             false,
                                                             payload);
