...
public:
  ...
  void playAudioFile(uint8_t audio_pin, AudioFile audio_file);
};
...
void AudioPlayer::playAudioFile(uint8_t audio_pin, AudioFile audio_file) {
  ...
  for (auto sample: audio_file.sound_data_) {
    hdw_ctl.analogWrite(audio_pin, sample)
  }
